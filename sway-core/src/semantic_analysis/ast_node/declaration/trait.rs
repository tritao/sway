...
use crate::{
    error::{err, ok},
    semantic_analysis::{Mode, TypeCheckArguments, TypedCodeBlock},
    type_engine::{insert_type, look_up_type_id, CopyTypes, TypeId, TypeMapping},
    CallPath, CompileError, CompileResult, FunctionDeclaration, FunctionParameter, Namespace,
    Purity, Supertrait, TraitDeclaration, TraitFn, TypeInfo, TypedDeclaration,
    TypedFunctionDeclaration, Visibility,
};
...
impl TypedTraitDeclaration {
    pub(crate) fn type_check(
        ...
        for trait_fn in trait_decl.interface_surface.into_iter() {
            interface_surface.push(check!(
                TypedTraitFn::type_check(trait_fn, &mut namespace, self_type),
                continue,
                warnings,
                errors
            ));
