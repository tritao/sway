...
fn types_are_subset_of(left: &[TypeInfo], right: &[TypeInfo]) -> bool {
    ...
    let mut subset_constraints = vec![];
    for i in 0..(right.len() - 1) {
        for j in (i + 1)..right.len() {
            ...
            if a.is_subset_of(b) {
                ...
                subset_constraints.push((i, j));
            }
            if b.is_subset_of(a) {
                // right[j] is a subset of right[i]
                subset_constraints.push((j, i));
            }
        }
    }
    for (i, j) in subset_constraints.into_iter() {
