use std::{
    cell::Cell, cmp::Ordering, collections::{BTreeSet, HashMap, HashSet}, fmt, hash::{DefaultHasher, Hash, Hasher}, sync::Arc
};
    ...
    pub(crate) fn check_orphan_rules(
        ...
        for key in keys {
            for trait_entry in trait_map.trait_impls[key].iter() {
                ...
                //     let args = arg.type_id.extract_inner_types(engines, IncludeSelf::Yes);
                ...
                'arg_loop: for arg in &trait_entry.key.name.suffix.args {
                    ...
                    arg.type_id.walk_inner_types(engines, IncludeSelf::Yes, &|inner_type_id| {
